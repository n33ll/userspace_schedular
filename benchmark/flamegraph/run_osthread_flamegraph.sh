...
SRC="$ROOT_DIR/benchmark/flamegraph/os_threads_runner.cpp"
OUT_DIR="$ROOT_DIR/benchmark/flamegraph/results/osthread_flame"
BIN="$OUT_DIR/../osthread_baseline_runner"
PERF_OUT="$OUT_DIR/osthread_runner.perf"
FOLDED_OUT="$OUT_DIR/osthread_runner.folded"
SVG_OUT="$OUT_DIR/osthread_runner.svg"
...
"$FLAMEGRAPH_PL" \
  --title "osthread_runner (profiling build)" \
  --countname samples \
  "$FOLDED_OUT" > "$SVG_OUT"
